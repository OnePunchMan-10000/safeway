...
// Accept emergency alert (volunteer)
...
// Reject emergency alert (volunteer)
...
router.put('/resolve/:alertId', async (req, res) => {
  try {
    ...
    // Only the victim or the assigned volunteer can resolve
    const canResolve = alert.userId.toString() === userId.toString() ||
                      (alert.acceptedBy.volunteerId && alert.acceptedBy.volunteerId.toString() === userId.toString())
    ...
    await alert.addTimelineEvent('resolved', userId, notes)
...
// Cancel emergency alert (only by the victim)
router.put('/cancel/:alertId', requireRole('woman'), async (req, res) => {
  try {
    const { alertId } = req.params
    const userId = req.user._id
    ...
    await alert.addTimelineEvent('cancelled', userId, 'Cancelled by user')

    // Notify volunteers
    const io = req.app.get('io') || global.io
    if (io) {
      io.to('volunteers').emit('alert-cancelled', {
        alertId: alert._id
      })
    }
...
router.get('/stats', requireRole(['volunteer']), async (req, res) => {
  try {
    ...
    const result = {
      ...
      todayAlerts: stats[0].todayAlerts[0]?.count || 0,
      activeAlerts: stats[0].activeAlerts[0]?.count || 0,
      resolvedAlerts: stats[0].resolvedAlerts[0]?.count || 0,
